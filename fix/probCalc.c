...
struct fractions recalculatePossibilities(struct fractions probs,
    ...
    for (unsigned int k = 0; k < NUMBER_PROBABILITIES; k++) {
        ...
        struct fraction resProb = resProbs.frac[k];
        unsigned int resNum = resProb.num;
        unsigned int resDenom = resProb.den;
        ...
        if (num && WEAK_SECURITY) {
             ...
             */
            // Update denominator.
            resProbs.frac[k].den = resDenom * denom * permSetSize;
            // Update numerator.
            resProbs.frac[k].num = (resNum * denom * permSetSize) + (resDenom * num);
        }
    }
    return resProbs;
}
