...
struct state alignAndAssignFractions(struct state resultState,
                                     struct fractions probs) {
    ...
    for (unsigned int j = 0; j < NUMBER_POSSIBLE_SEQUENCES; j++) {
        for (unsigned int k = 0; k < NUMBER_PROBABILITIES; k++) {
            resultState.seq[j].probs.frac[k].num *= newDenominator;
            resultState.seq[j].probs.frac[k].den *= probs.frac[k].num;
        }
    }
...
struct fractions computeTurnProbabilities(struct state resultState) {
    struct fractions probs;
    for (unsigned int i = 0; i < NUMBER_PROBABILITIES; i++) {
        probs.frac[i].num = 0;
...
struct turnStates applyTurn(struct state s) {
    ...
    if (WEAK_SECURITY) { // Weaker security check: output-possibilistic or input-possibilistic.
        ...
        for (unsigned int i = 0; i < MAX_TURN_OBSERVATIONS; i++) {
            if (result.isUsed[i]) {
                struct fractions probs = computeTurnProbabilities(result.states[i]);
                ...
                printf("\n");
                result.states[i] = alignAndAssignFractions(result.states[i], probs);
            }
        }
    }
